/////////////////////////////////// CREO UNA FUNCIÓN PARA POBLAR EL CONTENIDO DE LAS TABLAS /////////////////////////////////////////////
 ...
 function pueblaTabla(datos,campoclave,tabla,listaaplicaciones){
        ...
        datos.forEach(function(registro){                                       // Como datos es un array, hago un forEach para repasarlo
         ...
         // Función para mostrar el JSON en párrafos
        function mostrarJSON(json) {
            console.log('datos dentro: ',json);
            
            JSON.stringify(datos, null, 2); // Mostrar JSON formateado
            ...
            for (const key in json) {
                if (Array.isArray(json[key])) {
                    // Si el valor es un array, se muestra como una lista separada por comas
                    outputElement.innerHTML += `<p><span class="json-key">${key}:</span> ${json[key].join(", ")}</p>`;
                } else {
                    // Si el valor es un dato simple, se muestra directamente
                    outputElement.innerHTML += `<p><span class="json-key">${key}:</span> ${json[key]}</p>`;
                }
            }
        }
